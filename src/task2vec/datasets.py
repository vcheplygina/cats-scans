...
def inat2018(root, config):
    # from src.task2vec.dataset.inat import iNat2018Dataset
...
def cub_inat2018(root, config):
    ...
    if 0 <= config.task_id < NUM_CUB:
        # CUB
        # from src.task2vec.dataset.cub import CUBTasks, CUBDataset
...
def imat2018fashion(root, config):
    ...
    # from src.task2vec.dataset.imat import iMat2018FashionDataset, iMat2018FashionTasks
...
def split_mnist(root, config):
    ...
    # from src.task2vec.dataset.mnist import MNISTDataset, SplitMNISTTask
...
def split_cifar(root, config):
    ...
    # from src.task2vec.dataset.cifar import CIFAR10Dataset, CIFAR100Dataset, SplitCIFARTask
...
def cifar10_mnist(root, config):
    # from src.task2vec.dataset.cifar import CIFAR10Dataset
    # from src.task2vec.dataset.mnist import MNISTDataset
    # from src.task2vec.dataset.expansion import UnionClassificationTaskExpander
...
def isic2018(root, config):
    from src.task2vec.dataset.isic2018 import ISIC2018Dataset
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
...
def chest(root, config):
    from src.task2vec.dataset.chest import ChestDataset
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
...
def stl10_catsscans(root, config):
    from src.task2vec.dataset.stl10 import STL10Dataset
    transform = transforms.Compose([
...
def sti10(root, config):
    from src.task2vec.dataset.sti10 import STI10Dataset
    transform = transforms.Compose([
        transforms.ToTensor(),  # first create tensor from numpy array
        transforms.Resize(224),  # then resize array from 96x96to 224x224
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
...
def textures(root, config):
    from src.task2vec.dataset.textures import DTDDataset
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
...
def PCam(root, config):
    from src.task2vec.dataset.PCAM import PCAMDataset
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),
...
def kimia(root, config):
    from src.task2vec.dataset.kimia import KimiaDataset
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),

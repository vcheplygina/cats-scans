...
def import_blood(data_dir):
    ...
    for type_set in types:
        if type_set == '.DS_Store':
            ...
            train_image = [os.path.join(train_sub_folder, f) for f in os.listdir(train_sub_folder) if f.endswith('.jpeg')]
    ...
    train_labels = pd.concat(train_tables, ignore_index=True)  # create dataframe from list of tables and reset index
    print(train_labels['class'].value_counts())  # get information on distribution of labels in dataframe
    ...
    return train_labels, test_tables
...
def collect_data(isic, blood, img_dir, label_dir, test_size):
    ...
    """
    if isic:
        dataframe = import_ISIC(img_dir, label_dir, test_size)
